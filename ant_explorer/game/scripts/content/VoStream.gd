class_name VoStream
extends RefCounted
## Load chamber/role VO audio. Works even before Godot imports the WAV
## (Piper writes raw PCM files that ResourceLoader ignores until --import).
...
static func resolve_vo(dir: String, key: String) -> String:
	for ext in ["wav", "ogg"]:
...
static func _load_wav_file(path: String) -> AudioStreamWAV:
	...
	if bytes.size() < 44:
		return null
	# Minimal RIFF/WAVE PCM parser (Piper output).
