#!/usr/bin/env bash
# Full chamber + interaction QA suite for Ant Explorer (agents).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
GAME="$ROOT/game"
OUT="$ROOT/qa/out/chamber_suite"
mkdir -p "$OUT"
...
ARGS=(--path "$GAME" --fixed-fps 24 -s res://tools/chamber_interaction_suite.gd)
RUNNER=()
...
echo ">> chamber interaction suite"
"${RUNNER[@]}" "$GODOT" "${ARGS[@]}" "$@"
STATUS=$?
