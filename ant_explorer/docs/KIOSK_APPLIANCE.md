...
## What's active now
- **Home app:** `com.dylan.star_learner` (five-tile Star Learner launcher)
...
- **Lock task:** enterprise `LOCKED` via device owner (`com.dylan.star_learner`) — no “App is pinned” chrome. Re-enable with `tools/enable_device_owner.sh` + `tools/kiosk_on.sh` (do **not** use `am task lock`; that is consumer screen pinning).
...
adb shell cmd role remove-role-holder android.app.role.HOME com.dylan.star_learner
...
Do not replace the launcher with a game APK: `com.dylan.star_learner` is permanently the
