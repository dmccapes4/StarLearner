...
func start() -> bool:
	...
	if _recording:
		...
		_recording = false
	_mute_speakers(true)
	# New effect so get_recording() cannot return a prior buffer; player stays up.
	_rebuild_record_effect()
	_effect.set_recording_active(true)
	_recording = true
	return true

func stop_to_file(filename: String = "clip.wav") -> String:
	if not _recording:
		return ""
	_effect.set_recording_active(false)
	_recording = false
	...
	if stream == null:
		...
		return ""
	if _is_silent(stream):
		recording_failed.emit("silent")
		push_warning("MicCapture: silent clip (peak too low)")
		return ""
...
func _is_silent(stream: AudioStreamWAV) -> bool:
