class_name WriteFromNarration
extends Control
const ChromeIcons := preload("res://scripts/ChromeIcons.gd")
## Write → Narration: no cue image. Intro spells the word; clear icon replays
## the current letter (reveal after 3 wrongs or 3 clear taps).
...
func start() -> void:
	...
	Narrator.speak(LangVo.line("write_narration_blurb", _lang))
...
func _build() -> void:
	...
	header.texture = ChromeIcons.texture("narration")
	...
	_pick_row.add_theme_constant_override("separation", 20)
	...
	_word_banner = Label.new()
	_word_banner.add_theme_font_size_override("font_size", 42)
	_word_banner.add_theme_color_override("font_color", LangTheme.GOLD)
	_word_banner.position = Vector2(180, 55)
	_word_banner.size = Vector2(920, 50)
	_word_banner.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_word_banner.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_practice.add_child(_word_banner)

	_slots = LetterSlotsS.new()
	_slots.position = Vector2(340, 120)
	_slots.size = Vector2(600, 70)
	_practice.add_child(_slots)

	_board = AlphabetBoardS.new()
	_board.position = Vector2(320, 200)
	_board.size = Vector2(640, 300)
	_board.letter_pressed.connect(_on_letter)
	_board.clear_hint_pressed.connect(_on_clear_hint)
	_practice.add_child(_board)

	_trace = TraceCanvasS.new()
	_trace.position = Vector2(140, 200)
	_trace.size = Vector2(1000, 220)
	_trace.letter_traced.connect(_on_traced)
	_trace.ask_replay.connect(_on_clear_hint)
...
func _begin_word(word_id: String) -> void:
	...
	_word_banner.text = str(_word.get("word", ""))
	_session = WriteSessionS.new()
	_session.start(_word)
	_session.letter_advanced.connect(func(_i: int) -> void: _slots.set_filled(_session.filled_count()))
	...
	if use_sketch:
		...
		_board.setup(_lang, _session.expected_is_upper(), true)
		_board.reset_attempt(_session.expected_is_upper(), true)
		_hint.text = LangVo.line("tap_clear_letter", _lang)
	_intro()
...
func _intro() -> void:
	...
	var gen := _gen
	_busy = true
	# Spell letter-by-letter, then full word.
	for ch in _session.letters:
		if gen != _gen:
			return
		var d := Narrator.speak(LangLetters.letter_name(str(ch), _lang))
		if not await _wait(gen, maxf(0.55, d - 0.1)):
			return
	var spoken := str(_word.get("narration_word", _word.get("word", "")))
	var d2 := Narrator.speak(spoken)
	if not await _wait(gen, maxf(1.0, d2)):
		return
	var tip := LangVo.line("tap_clear_letter", _lang)
	d2 = Narrator.speak(tip)
	await _wait(gen, maxf(1.2, d2))
	_busy = false

func _on_clear_hint() -> void:
	...
	var d := Narrator.speak(LangLetters.letter_name(ch, _lang))
...
func _on_traced(_idx: int, letter: String) -> void:
	...
	var gen := _gen
	_busy = true
	var d := Narrator.speak(LangLetters.letter_name(letter, _lang))
...
func _on_wrong_feedback(glyph: String) -> void:
	...
	if not await _wait(gen, maxf(1.0, d)):
		return
	d = Narrator.speak(LangLetters.letter_name(_session.expected_letter(), _lang))
	await _wait(gen, maxf(0.7, d))
	_busy = false
...
func _on_word_finished() -> void:
	...
	Save.record_activity_finished("write_narr_" + str(_word.get("id", "")))
	_finish_beat()
...
static func vo_lines() -> Array:
	var out: Array = []
	out.append(LangVo.line("write_narration_blurb", "en"))
	out.append(LangVo.line("write_narration_blurb", "es"))
	out.append(LangVo.line("tap_clear_letter", "en"))
	out.append(LangVo.line("tap_clear_letter", "es"))
