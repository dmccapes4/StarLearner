extends SceneTree
## One-shot probe: reproduce the icon-marker sizing at a given trip fraction
## and print per-body distance / marker world size / expected on-screen px.
...
func _init() -> void:
	...
	for u_time in [0.15, 0.40, 0.70, 0.97]:
		...
		for b in SolarData.flyer_bodies(cfg):
			...
			var dist: float = maxf(cam.distance_to(pos), 0.001)
			var tier: float = SolarData.icon_tier_for(b)
			var world := OrbitMath.marker_world_size(dist, tier, cfg)
			var px: float = world / (2.0 * dist * tan(deg_to_rad(FOV_V * 0.5))) * VIEW_H
			print("  %-14s dist=%6.1f  marker=%5.2f  px=%5.1f" % [
				b["id"], dist, world, px])
	quit(0)
