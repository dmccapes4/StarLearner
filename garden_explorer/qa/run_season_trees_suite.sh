#!/usr/bin/env bash
# Trees + seasonal FX suite — screenshots + report under qa/out/season_trees/
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
GAME="$ROOT/game"
OUT="$ROOT/qa/out/season_trees"
mkdir -p "$OUT"
...
ARGS=(--path "$GAME" --fixed-fps 24 -s res://tools/season_trees_suite.gd)
RUNNER=()
...
echo ">> season trees suite via ${RUNNER[*]:-} $GODOT ${ARGS[*]}"
"${RUNNER[@]}" "$GODOT" "${ARGS[@]}" "$@"
STATUS=$?
...
print("agent_brief:", (d.get("agent_brief") or "")[:220], "...")
for c in bad:
