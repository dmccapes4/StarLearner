#!/usr/bin/env bash
# Bed + plant render suite — screenshots + report under qa/out/bed_plants/
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
GAME="$ROOT/game"
OUT="$ROOT/qa/out/bed_plants"
mkdir -p "$OUT"
...
ARGS=(--path "$GAME" --fixed-fps 24 -s res://tools/bed_plants_suite.gd)
RUNNER=()
...
echo ">> bed plants suite via ${RUNNER[*]:-} $GODOT ${ARGS[*]}"
"${RUNNER[@]}" "$GODOT" "${ARGS[@]}" "$@"
STATUS=$?
