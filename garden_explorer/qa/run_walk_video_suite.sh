#!/usr/bin/env bash
# Capture ~7s yard-walk clips + state.jsonl, mux to mp4, optional vision review.
#
#   ./qa/run_walk_video_suite.sh
#   REVIEW=0 ./qa/run_walk_video_suite.sh          # capture only
#   REVIEW=1 ./qa/run_walk_video_suite.sh          # capture + Grok/OpenAI review
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
GAME="$ROOT/game"
OUT_ROOT="$ROOT/qa/out/walk_video"
mkdir -p "$OUT_ROOT"
...
ARGS=(--path "$GAME" --fixed-fps 24 -s res://tools/walk_video_suite.gd)
RUNNER=()
...
echo ">> walk video suite via ${RUNNER[*]:-} $GODOT ${ARGS[*]}"
...
pathlib.Path("/tmp/walk_video_failed").write_text(str(len(failed)))
PY
  FAILED="$(cat /tmp/walk_video_failed 2>/dev/null || echo 1)"
fi
...
if [[ "${REVIEW:-1}" == "1" ]]; then
  ...
  python3 "$ROOT/qa/review_walk_videos.py" "$LATEST" || FAILED=1
fi
