#!/usr/bin/env python3
"""Bake Garden Explorer narration WAVs with ElevenLabs (shared family key).

  godot --headless --path game -s res://tools/dump_vo_lines.gd
  ./tools/gen_garden_vo.py
"""
...
MANIFEST = ROOT / "game" / "data" / "garden_vo_manifest.json"
...
def api_key() -> str:
    ...
    if not key:
        sys.exit(f"ERROR: ELEVENLABS_API_KEY missing — put it in {SECRETS}")
    return key
...
def main() -> None:
    ...
    print(f"{len(entries)} sentences; {len(todo)} to synthesize (~{sum(len(v) for v in todo.values())} chars).")
    n = 0
    with tempfile.TemporaryDirectory() as td:
        for key, text in sorted(todo.items()):
            mp3 = Path(td) / f"{key}.mp3"
            wav = VO_DIR / f"{key}.wav"
            print(f"  [{n + 1}/{len(todo)}] {text[:60]}")
            synth_mp3(text, mp3)
            to_wav(mp3, wav)
            n += 1
    print(f"OK — baked {n} clips into {VO_DIR}")
